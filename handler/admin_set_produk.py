from aiogram import Router, F
from aiogram.types import CallbackQuery
from button.admin_set_produk import get_admin_set_produk_keyboard
from button.start import get_admin_keyboard  # tombol start admin

router = Router()

@router.callback_query(F.data == "seting_produk")
async def admin_set_produk_handler(callback: CallbackQuery):
    await callback.message.edit_text(
        "<b>🎁 Menu Seting Produk</b>\n\n"
        "Silakan pilih aksi yang ingin dilakukan untuk produk:",
        parse_mode="HTML",
        reply_markup=get_admin_set_produk_keyboard()
    )
    await callback.answer()

@router.callback_query(F.data == "admin_start")
async def back_to_admin_menu_handler(callback: CallbackQuery):
    await callback.message.edit_text(
        "<b>Selamat datang di Menu Admin!</b>\n"
        "Silakan pilih menu yang tersedia di bawah ini:",
        parse_mode="HTML",
        reply_markup=get_admin_keyboard()
    )
    await callback.answer()